...
const MyProfile = () => {
  ...
  const [userData, setUserData] = useState({
    ...
    email : 'officerk@gmail.com',
    phone : '+91 XXXXX XXXXX',
    address : {
      line1 : '11XX96 SK App.',
      line2 : 'Suite 304, Gotham City, USA'
    },
    gender : 'Male',
    dob : '2004-09-10'
  })
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      <div className='mt-10'>
        {
          isEdit
          ? <button className='border border-[#96be25] px-8 py-2 rounded-full hover:bg-[#96be25] hover:text-white transition-all cursor-pointer' onClick={()=>setIsEdit(false)}>Save Information</button>
          : <button className='border border-[#96be25] px-8 py-2 rounded-full hover:bg-[#96be25] hover:text-white transition-all cursor-pointer' onClick={()=>setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  )
